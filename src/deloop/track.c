...
void deloop_track_ringbuf_read(deloop_track_ringbuf_t *rb, float *data, size_t cnt) {
  if ((rb == NULL) || (data == NULL)) return 0;
  if (rb->read >= rb->size) {
    rb->read = 0;
  }
...
deloop_track_t *deloop_track_create(deloop_track_type_t type, jack_nframes_t frame_rate) {
...
void deloop_track_free(deloop_track_t *track) {
  ...
  if (track->lrb != NULL) deloop_track_ringbuf_free(track->lrb);
  if (track->rrb != NULL) deloop_track_ringbuf_free(track->rrb);
  free(track);
}
...
void deloop_track_stereo_tick(
  ...
  if (deloop_track_is_recording(track)) {
    if (lin != NULL) {
      ...
      deloop_track_ringbuf_write(track->lrb, lin, cnt, track->state == deloop_TRACK_STATE_OVERDUBBING);
    }

    if (rin != NULL) {
      assert(track->rrb != NULL && track->rrb->buf != NULL);
      deloop_track_ringbuf_write(track->rrb, rin, cnt, track->state == deloop_TRACK_STATE_OVERDUBBING);
    }
  }
...
void deloop_track_stereo_read(
  ...
  float *rout,
  size_t cnt) {
  if ((track == NULL) || track->state == deloop_TRACK_STATE_STOPPED) return;
  assert(track->type == deloop_TRACK_TYPE_STEREO);
